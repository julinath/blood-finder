...
import { DonorDetailsSection, ProfileSection, SignOutButton } from './_components'
import {
  AvailabilityToggle,
  CancelRequestButton,
    ...
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <div className="mb-8">
        <ProfileSection
          profile={{
            ...
            email: displayEmail(profile?.email ?? user.email ?? ''),
      ...
      {donor && (
        ...
        >
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  ...
                  {donor.donation_count > 0 &&
                    ` · 🩸 ${toBnDigits(donor.donation_count)} বার রক্ত দিয়েছেন`}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
          ...
          <DonationEligibilityNote lastDonationDate={donor.last_donation_date} />
          <DonorDetailsSection
            donor={{
              ...
              weight_kg: donor.weight_kg ? String(donor.weight_kg) : '',
                            ...
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <div className="flex items-center gap-2">
                                {req.status === 'OPEN' && (
                                  <CreditDonorButton
                                    requestId={req.id}
                                    donorUserId={offer.donor_id}
                                    donorName={offer.donor?.full_name ?? 'Donor'}
                                  />
                                )}
                                <ReportButton
                                  requestId={req.id}
...
// 90-day-gap verdict shown to the donor on their own card, so they know at a
// glance whether they can donate right now.
function DonationEligibilityNote({
  lastDonationDate,
}: {
  ...
  if (eligibility.isEligible) {
    return (
      <p className="text-sm text-green-700 mt-4 bg-green-100 rounded-xl px-4 py-2.5">
  ...
  return (
    <p className="text-sm text-amber-700 mt-4 bg-amber-100 rounded-xl px-4 py-2.5">
      ⏳ শেষ রক্তদানের পর ৯০ দিন পূর্ণ হয়নি — আরো{' '}
      {toBnDigits(eligibility.daysRemaining)} দিন পরে (
      {formatBnDate(eligibility.eligibleOn.toISOString().slice(0, 10))}) আবার
      রক্ত দিতে পারবেন।
    </p>
  )
}
